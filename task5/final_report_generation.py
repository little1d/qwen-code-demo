...
def create_main_outcome_stats_table(data):
    ...
    if discontinuation_analysis is not None:
        discontinuation_rate_a = (discontinuation_analysis[
        ...
        discontinuation_rate_b = (discontinuation_analysis[
        ...
        # 查找Discontinuation Rate行并更新数据
        mask = main_outcomes_summary['Outcome'] == 'Discontinuation Rate'
        if mask.any():
            main_outcomes_summary.loc[mask, 'DRUG_A_Mean'] = round(discontinuation_rate_a, 3)
            main_outcomes_summary.loc[mask, 'DRUG_B_Mean'] = round(discontinuation_rate_b, 3)
            main_outcomes_summary.loc[mask, 'Difference'] = round(discontinuation_rate_a - discontinuation_rate_b, 3)
        else:
            # 如果没有Discontinuation Rate行，则添加它
            new_row = pd.DataFrame({
                'Outcome': ['Discontinuation Rate'],
                'DRUG_A_Mean': [round(discontinuation_rate_a, 3)],
                'DRUG_B_Mean': [round(discontinuation_rate_b, 3)],
                'Difference': [round(discontinuation_rate_a - discontinuation_rate_b, 3)]
            })
            main_outcomes_summary = pd.concat([main_outcomes_summary, new_row], ignore_index=True)
...
## 主要发现
基于{len(data['outcomes'])}名患者的分析结果如下：

### 医疗资源利用
- 急诊就诊次数：DRUG_A组均值{main_outcomes_summary[main_outcomes_summary['Outcome'] == 'ER Visits']['DRUG_A_Mean'].values[0]}次 vs DRUG_B组{main_outcomes_summary[main_outcomes_summary['Outcome'] == 'ER Visits']['DRUG_B_Mean'].values[0]}次
- 住院次数：DRUG_A组均值{main_outcomes_summary[main_outcomes_summary['Outcome'] == 'Hospitalizations']['DRUG_A_Mean'].values[0]}次 vs DRUG_B组{main_outcomes_summary[main_outcomes_summary['Outcome'] == 'Hospitalizations']['DRUG_B_Mean'].values[0]}次
- 门诊就诊次数：DRUG_A组均值{main_outcomes_summary[main_outcomes_summary['Outcome'] == 'Outpatient Visits']['DRUG_A_Mean'].values[0]}次 vs DRUG_B组{main_outcomes_summary[main_outcomes_summary['Outcome'] == 'Outpatient Visits']['DRUG_B_Mean'].values[0]}次

### 成本分析
- 总医疗费用：DRUG_A组均值{main_outcomes_summary[main_outcomes_summary['Outcome'] == 'Total Cost']['DRUG_A_Mean'].values[0]:.0f}元 vs DRUG_B组{main_outcomes_summary[main_outcomes_summary['Outcome'] == 'Total Cost']['DRUG_B_Mean'].values[0]:.0f}元
- 处方费用：DRUG_A组均值{main_outcomes_summary[main_outcomes_summary['Outcome'] == 'Prescription Cost']['DRUG_A_Mean'].values[0]:.0f}元 vs DRUG_B组{main_outcomes_summary[main_outcomes_summary['Outcome'] == 'Prescription Cost']['DRUG_B_Mean'].values[0]:.0f}元

### 治疗依从性与持续性
- PDC≥80%患者比例：DRUG_A组{main_outcomes_summary[main_outcomes_summary['Outcome'] == 'PDC≥80% Proportion']['DRUG_A_Mean'].values[0]*100:.1f}% vs DRUG_B组{main_outcomes_summary[main_outcomes_summary['Outcome'] == 'PDC≥80% Proportion']['DRUG_B_Mean'].values[0]*100:.1f}%
- 停药率：DRUG_A组{main_outcomes_summary[main_outcomes_summary['Outcome'] == 'Discontinuation Rate']['DRUG_A_Mean'].values[0]*100:.1f}% vs DRUG_B组{main_outcomes_summary[main_outcomes_summary['Outcome'] == 'Discontinuation Rate']['DRUG_B_Mean'].values[0]*100:.1f}%

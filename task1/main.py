...
def save_outputs(final_study_population, datasets, output_dir='task1'):
    """
    保存清理后的数据集、符合条件的患者队列列表、基线期和随访期定义
    """
    print("保存输出文件...")
    ...
    # 保存最终研究队列
    output_file = os.path.join(output_dir, 'study_population.csv')
    final_study_population.to_csv(output_file, index=False)
    print(f"研究队列已保存至: {output_file}")
    
    # 保存每个数据集的清理版本
    for name, df in datasets.items():
        output_file = os.path.join(output_dir, f'cleaned_{name}.csv')
        df.to_csv(output_file, index=False)
        print(f"清理后的{name}数据已保存至: {output_file}")
    
    # 生成符合条件的患者队列列表
    eligible_patients_list = final_study_population[['patient_id', 'treatment_group']].copy()
    eligible_patients_list['baseline_start'] = final_study_population['baseline_start'].dt.strftime('%Y-%m-%d')
    eligible_patients_list['baseline_end'] = final_study_population['baseline_end'].dt.strftime('%Y-%m-%d')
    eligible_patients_list['followup_start'] = final_study_population['followup_start'].dt.strftime('%Y-%m-%d')
    eligible_patients_list['followup_end'] = final_study_population['followup_end'].dt.strftime('%Y-%m-%d')
    
    output_file = os.path.join(output_dir, 'eligible_patients_list.csv')
    eligible_patients_list.to_csv(output_file, index=False)
    print(f"符合条件的患者队列列表已保存至: {output_file}")
...
if __name__ == "__main__":
    ...
    # 保存输出
    save_outputs(final_study_population, datasets)

...
if __name__ == "__main__":
    ...
    # 保存高依从性患者列表
    good_adherence_patients.to_csv('task4/tables/good_adherence_patients.csv', index=False)
    
    # 创建依从性总结表
    adherence_summary = pd.DataFrame({
        'metric': ['Total Patients', 'Good Adherence (PDC≥80%)', 'Poor Adherence (PDC<80%)', 'Good Adherence Percentage'],
        'value': [len(pdc_df), len(good_adherence_patients), len(pdc_df) - len(good_adherence_patients), good_adherence_pct]
    })
    adherence_summary.to_csv('task4/tables/adherence_summary.csv', index=False)
    
    print("PDC结果已保存至 task4/tables/ 目录")
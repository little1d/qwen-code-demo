...
def perform_kaplan_meier_analysis(discontinuation_df):
    ...
    for treatment in treatment_groups:
        ...
        # 计算180天生存概率
        try:
            survival_at_180 = kmf.survival_function_at_times([180])
            if len(survival_at_180) > 0:
                # 使用索引访问值
                survival_index = survival_at_180.index[0]
                survival_prob = survival_at_180.loc[survival_index]
            else:
                survival_prob = 'N/A'
        except:
            survival_prob = 'N/A'
        ...
        print(f"  180天生存概率: {survival_prob}")
...
def generate_visualizations(results):
    ...
    resource_data = outcomes.groupby('treatment_group')[['er_visits', 'hospitalizations', 'outpatient_visits']].mean()
    
    plt.figure(figsize=(12, 8))
    
    # 子图1: 医疗资源利用
    plt.subplot(2, 2, 1)
    resource_data.plot(kind='bar', ax=plt.gca())
    plt.title('平均医疗资源利用 - 按治疗组')
    plt.ylabel('次数')
    ...
    # 子图2: 成本比较
    plt.subplot(2, 2, 2)
    cost_data = outcomes.groupby('treatment_group')[['total_cost', 'prescription_cost']].mean()/1000  # 单位转换为千
    cost_data.plot(kind='bar', ax=plt.gca())
    plt.title('平均费用 (千) - 按治疗组')
    plt.ylabel('费用 (千)')
    plt.xticks(rotation=45)
    plt.legend()
    plt.grid(axis='y', alpha=0.3)
    ...
    # 子图3: PDC分布
    plt.subplot(2, 2, 3)
    pdc_data = results['pdc_df']
    for treatment in pdc_data['treatment_group'].unique():
        group_data = pdc_data[pdc_data['treatment_group'] == treatment]['pdc']
        plt.hist(group_data, alpha=0.7, label=treatment, bins=15)
    plt.title('PDC分布 - 按治疗组')
    plt.xlabel('PDC (%)')
    plt.ylabel('频数')
    plt.legend()
    plt.grid(axis='y', alpha=0.3)
    ...
    # 子图4: 停药率
    plt.subplot(2, 2, 4)
    discontinuation_data = results['discontinuation_df']
    discontinuation_rates = []
    labels = []
    for treatment in discontinuation_data['treatment_group'].unique():
        group_data = discontinuation_data[discontinuation_data['treatment_group'] == treatment]
        rate = (group_data[group_data['censored'] == 0].shape[0] / group_data.shape[0]) * 100
        discontinuation_rates.append(rate)
        labels.append(treatment)
    
    plt.bar(labels, discontinuation_rates)
    plt.title('停药率 - 按治疗组')
    plt.ylabel('停药率 (%)')
    plt.grid(axis='y', alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('task4/figures/comprehensive_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    # 2. 生成PDC对比箱线图
    plt.figure(figsize=(10, 6))
    pdc_data = results['pdc_df']
    plt.boxplot([pdc_data[pdc_data['treatment_group'] == 'DRUG_A']['pdc'], 
                 pdc_data[pdc_data['treatment_group'] == 'DRUG_B']['pdc']], 
                labels=['DRUG_A', 'DRUG_B'])
    plt.title('PDC分布箱线图 - 按治疗组')
    plt.ylabel('PDC (%)')
    plt.grid(axis='y', alpha=0.3)
    plt.savefig('task4/figures/pdc_boxplot.png', dpi=300, bbox_inches='tight')
    plt.close()

...
def generate_descriptive_stats_tables(baseline_demo, comorbidities, resource_utilization, datasets):
    ...
    # 添加基线期资源利用信息（不包括treatment_group以避免重复）
    ...
    # 合并所有数据，保留treatment_group列
    baseline_data = baseline_data.merge(
        baseline_with_events.drop(columns=['treatment_group'], errors='ignore'), 
        on='patient_id', 
        how='left'
    ).merge(
        baseline_with_costs.drop(columns=['treatment_group'], errors='ignore'), 
        on='patient_id', 
        how='left'
    )
...
def perform_statistical_comparison(baseline_summary):
    ...
    comparison_df.to_csv('task2/tables/comparison_results.csv')
    
    print(f"\n统计检验结果已保存至 task2/tables/comparison_results.csv")
